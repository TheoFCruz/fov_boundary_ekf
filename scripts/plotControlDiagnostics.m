...
if ~exist('result', 'var') || ~isstruct(result) || ...
        ~all(isfield(result, {'Time', 'ObserverInput', 'ObserverPose', 'PolicyDiagnostics'}))
    error('scripts:plotControlDiagnostics:MissingResult', ...
        'Run a scenario first so its simulation result is available as result.');
end
if numel(result.PolicyDiagnostics) ~= numel(result.Time) - 1 || ...
        ~all(cellfun(@isstruct, result.PolicyDiagnostics))
    error('scripts:plotControlDiagnostics:InvalidDiagnostics', ...
        'result must contain one struct PolicyDiagnostics entry per input interval.');
end
requiredFields = {'Status', 'BarrierValue', 'Slack'};
if ~all(cellfun(@(diagnostic) all(isfield(diagnostic, requiredFields)), ...
        result.PolicyDiagnostics))
    error('scripts:plotControlDiagnostics:MissingControlDiagnostics', ...
        'result.PolicyDiagnostics does not contain CBF-QP diagnostics.');
end
...
disp(table(statusNames, statusCounts, 'VariableNames', {'Status', 'Count'}));
